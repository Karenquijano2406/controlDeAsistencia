...
if (!empty($_GET["txtfechainicio"]) and !empty($_GET["txtfechafinal"]) and !empty($_GET["txtempleado"])) {
   ...
   class PDF extends FPDF
   {
      ...
      function Header()
      {
         ...
         $this->Image('logoTuul.jpg', 270, 5, 20); //logo de la empresa,moverDerecha,moverAbajo,tamañoIMG
         ...
         /* CAMPOS DE LA TABLA */
         //color
         $this->SetFillColor(242, 173, 13); //colorFondo
   ...
   if ($empleado == "todos") {
      $sql = $conexion->query("SELECT
          ...
          date_format(asistencia.entrada, '%m-%d-%Y') as entrada,
          date_format(asistencia.salida, '%m-%d-%Y') as salida,
   ...
   while ($datos_reporte=$sql->fetch_object()) {
      ...
      $pdf->Cell(30, 10, utf8_decode($datos_reporte->totalHR), 1, 1, 'C', 0); //muestra el total de horas
   }
